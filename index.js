...
app.post("/userpost", async (req, res) => {
  const repId = req.body.UserId
  try {
    const allPost = await posts.findAll({ where: { UserId: repId } });
    res.send(allPost);
  } catch (error) {
    res.json("there was an error:", error.message);
  }
...
app.post("/user", async (req, res) => {
  try {
    ...
    const theUser = await users.findAll({ where: { username: username } });
    res.send(theUser[0].id);
  } catch (error) {
    res.json("there was an error 13:", error.message);
  }
});
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (username && password) {
      const queriedUserArray = await users.findAll({
...
app.put("/categoryList/:id", async (req, res) => {
  try {
    const reqId = Number(req.params.id);
    const { name } = req.body;
    await category.update({ name: name }, { where: { id: reqId } });
